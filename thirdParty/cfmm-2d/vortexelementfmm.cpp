...
void mexFunction( int nlhs, mxArray *leftSideData[], int nrhs, const mxArray *rightSideData[] ) { 
   ...
   if (nrhs != 11) 
   {
   	mexErrMsgTxt("Wrong number of input arguments. Is meant to be of form ([xl], [yl], [strl],[xr], [yr], [strr], [elalX], [elalY], precision, targetsPerBox, threads)"); 
   }
   if (nlhs != 2) 
   {
   ...
   double targetsPerBox	= *mxGetPr(rightSideData[9]);
   double maxThreads 	= *mxGetPr(rightSideData[10]);
    
    // Do the actual computations using my c function
    VortexElementFMM calc;
    calc.run( precision, targetsPerBox, maxThreads, xPosl , yPosl, strengthl, xPosr , yPosr, strengthr, xEvalPos, yEvalPos, outUvel, outVvel, numvorts, numevals);
